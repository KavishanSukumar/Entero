...
router.post("/", validInfo, async (req, res) => {
  try {
    ...
    const newCustomer = await pool.query(
      "INSERT INTO customer (userid, status) VALUES ($1, 'a')",
      [userid]
    );
    ...
    const newEmailToken=await pool.query(
      ...
      const link="http://localhost:3000/emailverify/"+userid+"/"+ newEmailToken.rows[0].token
...
router.post("/:userid/:token", async (req, res) => {
  try {
    ...
    const users = await pool.query("SELECT * FROM email_validate WHERE userid=$1 AND token=$2", [
      userid,token
    ]);
    if (users.rows.length === 0) {
