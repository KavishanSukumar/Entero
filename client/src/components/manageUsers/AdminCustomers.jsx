...
function AdminCustomers() {
  ...
  const [rawCustomerDetail, setrRawCustomerDetail] = useState([]);
  ...
  const searchByName = (e) => {
    if (e.target.value === "") {
      setCustomerDetail(rawCustomerDetail);
    } else {
      setCustomerDetail([]);
      rawCustomerDetail.map((item) => {
        if (
          item.name.toLowerCase().includes(e.target.value.toLowerCase()) ||
          item.userid === e.target.value
        ) {
          setCustomerDetail((customerDetail) => [...customerDetail, item]);
        }
      });
    }
  };
        ...
        <div className="overflow-y-auto bg-white fixed rounded-3xl shadow-2xl top-[20%] flex flex-col  w-[80%] mx-[10%] md:w-[60%] md:mx-[20%] lg:w-[40%] lg:mx-[30%] p-2">
          ...
          <div className="w-full flex flex-col lg:flex-row ">
            <div className="flex flex-col  items-center  mb-2 justify-center">
              {/*The image */}
              <div className="p-2">
                {customerDetail && customerDetail.image === null ? (
                  <img
              ...
              <div className="flex flex-col w-44 p-4">
                <button
                  className=" border-2 bg-cyan-500 hover:bg-cyan-400  p-1 mb-3 rounded text-white"
                  onClick={() => {
                    changeStatus(customerDetail.userid);
                  }}
                >
                  {customerDetail && customerDetail.status === "a"
                    ? "Remove"
                    : "Activate"}
                </button>
              </div>
            ...
            <div className=" w-full p-2">
              ...
              <dl class="text-gray-900 divide-y divide-gray-200 ">
                ...
                <div class="flex flex-col pt-3">
                  ...
                  <dd class="text-md font-semibold">
                    {customerDetail && customerDetail.contact_number}
                  </dd>
                </div>
              </dl>
            </div>
